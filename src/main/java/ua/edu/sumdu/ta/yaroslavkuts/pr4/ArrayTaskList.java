...
import ua.edu.sumdu.ta.yaroslavkuts.pr4.AbstractTaskList;
...
public class ArrayTaskList extends AbstractTaskList {
	...
	public ArrayTaskList() {
		numberOfLists++;
		list_id = DBConnector.insertList(this);
	}
	...
	public void add(Task task) {
		if (task != null) {
			if (taskList.length == size()) {
				...
				task.setTask_id(DBConnector.addTaskInList(task, this.list_id));
				size++;
			}
		...
		if (task != null) {
			for (int i = 0; i < size(); i++) {
				if (task.equals(taskList[i])) {
					...
					task.setTask_id(DBConnector.addTaskInList(task, null));
					size--;
					break;
				}
			}
