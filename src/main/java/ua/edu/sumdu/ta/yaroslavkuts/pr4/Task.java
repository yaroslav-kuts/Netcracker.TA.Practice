...
public class Task {
	...
	public Task() {
		super();
		task_id = DBConnector.insertTask(this);
	}
	...
	public final void setTitle(String title) {
		if (title != null && title != "") {
			this.title = title;
			task_id = DBConnector.modifyTask(this);
		} else {
			System.out.println("Incorrect value of task's title");
		}
	}
	...
	public final void setTime(int time) {
		...
		if (time > 0) {
			...
			task_id = DBConnector.modifyTask(this);
		} else System.out.println("The value of 'time' does not validate");
	}
	 ...
	 * @param repeat
	 */
	public final void setTime(int start, int end, int repeat) {
		if (!isRepeated()) {
			time = 0;
		}
